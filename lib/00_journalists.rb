...
def count_handles(hash)
    puts "Voici le nombre de journalistes : #{hash.size}"
end

def count_numbers (hash)
    i=0
    hash.each do |value|
        if value =~ /\d/
            i+=1
    ...
    puts "Il y a #{i} handles qui contiennent un numéro"
end

def count_AudeName (hash)
    i=0
    hash.each do |value|
        if value.match(/aude/)
            puts " nom #{value}"
...
count_numbers(journalists)
count_AudeName(journalists)
